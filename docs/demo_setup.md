...
- original fastq files were subsampled to 0.1% of original size
- we'll only align to Chromosome 5 for demonstration purpose
```bash
ls *.fastq.gz | parallel -j 15 'seqtk sample -s 777 {} 0.001 | gzip > sample/subsampled_{}'
```

1. Download sequence of Chr5 (Reference genome)
- [GRCh38's Chr5](https://hgdownload.soe.ucsc.edu/goldenPath/hg38/chromosomes/chr6.fa.gz)
```bash
